...
function tankMapCollision(tank,mapobj){
	...
	// alert(tileNum +"        " +mapobj.wTileCount+ "        " +colIndex + "        " +tank.y )

		for(var i=0;i<tileNum && colIndex+i < mapobj.wTileCount ;i++){
			var mapContent = map2[rowIndex][colIndex+i];

		//	 alert(rowIndex +"        " +colIndex + "        " +mapContent + "        " +tank.y )
			if(mapContent == 1 || mapContent == 2){
				if(tank.dir == UP){
		...
		for(var i=0;i<tileNum && rowIndex+i < mapobj.HTileCount;i++){
			var mapContent = map2[rowIndex+i][colIndex];
			if(mapContent == 1 || mapContent == 2){
				if(tank.dir == LEFT){
...
function bulletMapCollision(bullet,mapobj){
	...
	if(bullet.dir == UP || bullet.dir == DOWN){
		...
		for(var i=0;i<tileNum && colIndex+i < mapobj.wTileCount ;i++){
			var mapContent = map2[rowIndex][colIndex+i];
			if(mapContent == 1 || mapContent == 2 || mapContent == 9 ){
				//bullet.distroy();
				result = true;
				if(mapContent == 1 ){
					//墙被打掉
					mapChangeIndex.push([rowIndex,colIndex+i]);
				}else if(mapContent == 9){
					isGameOver = true;
					break;
				}
			}
